        ...
        return $form
            ->schema([
                ...
                Section::make('Customização no Código HTML BASE')
                    ->description('Customize seu css, js, código pixels ou adicione conteúdo no corpo da sua página')
                    ->collapsible()
                    ->collapsed(true)
                    ->schema([
                        CodeField::make('pixel_cadastro')
                            ->label("Código Pixel Cadastro")
                            ->setLanguage(CodeField::JS)
                            ->withLineNumbers()
                            ->minHeight(400),
                        CodeField::make('pixel_deposito')
                            ->label("Código Pixel Depósito")
                            ->setLanguage(CodeField::JS)
                            ->withLineNumbers()
                            ->minHeight(400),
                ...
                Section::make('Suporte')
                    ->description('Coloque um link de suporte')
                    ->collapsible()
                    ->collapsed(true)
                    ->schema([
                        TextInput::make("link_suporte")
                            ->label("Link do suporte")
                            ->placeholder('Digite a URL do seu suporte')
                            ->url()
                    ])->columns(2),
            ])
            ->statePath('data');
